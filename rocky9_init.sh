#!/bin/bash
# Optimized Rocky Linux 9 initialization script
set -euo pipefail
...
# Backing up existing repository configuration
echo -e "\033[34mBacking up existing repository configuration...\033[0m"
mkdir -p /etc/yum.repos.d/backup
cp -r /etc/yum.repos.d/*.repo /etc/yum.repos.d/backup/

# Configure Aliyun repositories for Rocky Linux 9
echo -e "\033[34mConfiguring Aliyun repositories for Rocky Linux 9...\033[0m"
cat <<EOL > /etc/yum.repos.d/Rocky-Base.repo
[rocky-baseos]
name=Rocky Linux \$releasever - BaseOS
baseurl=http://mirrors.aliyun.com/rockylinux/9/BaseOS/x86_64/os/
enabled=1
gpgcheck=0

[rocky-appstream]
name=Rocky Linux \$releasever - AppStream
baseurl=http://mirrors.aliyun.com/rockylinux/9/AppStream/x86_64/os/
enabled=1
gpgcheck=0

[rocky-extras]
name=Rocky Linux \$releasever - Extras
baseurl=http://mirrors.aliyun.com/rockylinux/9/extras/x86_64/os/
enabled=1
gpgcheck=0
EOL
...
dnf install -y net-tools lrzsz gcc gcc-c++ vim pcre pcre-devel openssl openssl-devel \
    ...
    libevent curl curl-devel nfs-utils git chrony iotop iftop
